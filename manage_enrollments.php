...
$result = null; // Initialize result to avoid undefined variable warning
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_status'])) {
    ...
    $valid_statuses = ['Not Started Yet', 'In Progress', 'Completed'];
    if (in_array($new_status, $valid_statuses)) {
        $update_status_sql = "UPDATE enrollments SET status = ? WHERE id = ?";
...
// Query to fetch enrollments including status
$sql = "SELECT id, user_email, course, start_date, Preferred_time, amount, payment_status, status FROM enrollments"; // Adjust this based on your enrollment table
            ...
            <tbody>
                ...
                if ($result && $result->num_rows > 0) {
                    while ($row = $result->fetch_assoc()) {
                        ...
                        echo "<input type='hidden' name='enrollment_id' value='" . htmlspecialchars($row['id']) . "'>";
                        echo "<select name='status' required>";
                        echo "<option value='Not Started Yet' " . ($row['status'] === 'Not Started Yet' ? 'selected' : '') . ">Not Started Yet</option>";
                    ...
                    echo "<tr><td colspan='9' class='no-data'>No enrollments found.</td></tr>";
                }
                ?>
            </tbody>
        </table>
